...
void main() {
  vec4 color0 = texture(sampler2D(texture0, linearSampler), vUv0);
  vec4 color1 = texture(sampler2D(texture1, linearSampler), vUv0);
  vec4 color = mix(color0, color1, 0.25);

  float lighting = SampleLightTable(texture3, normalize(vNormal.xyz), vUv0);
