...
void main() {
  vec4 texA = texture(sampler2D(texture0, linearSampler), vUv1);
  vec4 texC = texture(sampler2D(texture1, linearSampler), vUv2);
  vec4 texD = texture(sampler2D(texture2, linearSampler), vUv2);
