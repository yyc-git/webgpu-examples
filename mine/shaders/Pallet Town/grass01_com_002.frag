...
void main() {
  vec4 transition = texture(sampler2D(texture3, linearSampler), vUv2);
  vec4 noise = texture(sampler2D(texture2, linearSampler), vUv0);

  vec4 colorA = texture(sampler2D(texture1, linearSampler), vUv1);
  vec4 colorB = texture(sampler2D(texture4, linearSampler), vUv1);

  vec4 mask = texture(sampler2D(texture0, linearSampler), vUv1);
  if (mask.a <= 0.85) discard;

  outColor = transition * mix(
    vColor0,
    blend(colorA, colorB, noise.r),
    0.5
  );
