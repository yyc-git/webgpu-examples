...
void main() {
  vec4 transition = texture(sampler2D(texture0, linearSampler), vUv1);
  vec4 noise = texture(sampler2D(texture2, linearSampler), vUv0);
  vec4 mask = texture(sampler2D(texture3, linearSampler), vUv1);
  ...
  outColor = transition * blend(colorA, colorB, noise.r);
