...
void main() {
  ...
  vec4 texA = texture(sampler2D(texture2, linearSampler), vUv0);
  vec4 texB = texture(sampler2D(texture3, linearSampler), vUv0);
  vec4 texC = texture(sampler2D(texture1, linearSampler), vUv0);
  vec4 texD = texture(sampler2D(texture5, linearSampler), vUv0);
  vec4 noise = texture(sampler2D(texture0, linearSampler), vUv0);
  vec4 color = transition * blend(
    blend(texC, texD, noise.r),
