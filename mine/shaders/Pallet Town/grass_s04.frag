...
void main() {
  vec4 mask = texture(sampler2D(texture6, linearSampler), vUv1);

  vec4 noise = texture(sampler2D(texture4, linearSampler), vUv0);
  vec4 colorA = texture(sampler2D(texture2, linearSampler), vUv0);
  vec4 colorB = texture(sampler2D(texture3, linearSampler), vUv0);

  if (mask.a <= 0.85) discard;

  outColor = blend(colorA, colorB, noise.r);
